---
title: Mira Murati
description: OpenAI's chief technology officer through the ChatGPT and GPT-4 era, briefly its interim CEO during the 2023 board crisis, and later founder of Thinking Machines Lab.
tags: [executive]
aliases: [Murati]
updated: 2026-06-18
sources:
  - id: murati-wiki
    title: "Mira Murati (Wikipedia)"
    url: https://en.wikipedia.org/wiki/Mira_Murati
    publisher: Wikipedia
    year: 2025
  - id: openai-transition
    title: "OpenAI announces leadership transition"
    url: https://openai.com/index/openai-announces-leadership-transition/
    publisher: OpenAI
    year: 2023
---

# Mira Murati

**Mira Murati** was the chief technology officer of [[OpenAI]] through its most
consequential period, and briefly its interim CEO during the company's 2023 leadership
crisis. She has since founded her own AI company.

## CTO of OpenAI

Murati served as OpenAI's chief technology officer from 2022 to 2024, the stretch that
produced the company's defining products. In Wikipedia's summary, she "led OpenAI's
work on ChatGPT, Dall-E, Codex and Sora, while overseeing its research, product and
safety teams."[^murati-wiki] That put her at the technical and product helm of the
[[ChatGPT launch]] and the GPT-4 generation; the products themselves are covered under
[[OpenAI]].

## Interim CEO, November 2023

When OpenAI's board removed CEO [[Sam Altman]] on November 17, 2023, Murati "became
interim chief executive officer."[^murati-wiki][^openai-transition] She held the role
only briefly — another interim CEO was named within days, before Altman was reinstated
and she returned to her CTO post.[^murati-wiki] The full sequence is told in the
[[OpenAI board crisis]].

## Thinking Machines Lab

Murati announced her departure from OpenAI in September 2024, and in February 2025 she
launched **Thinking Machines Lab**, a new AI startup — where [[John Schulman]], another
OpenAI alumnus, became chief scientist.[^murati-wiki]
